...
class Inventory:
    ...
    def __init__(self, products, categories, history, _next_id):
    ...
    def add_product(self, product: Product) -> int:
        ...
        """
        self.products.get[self._next_id] = product
    ...
    def remove_product(self, product_id: int) -> Product:
        """TODO : Remove a product by ID and return it.
    ...
    def get_product(self, product_id: int) -> Product:
        """TODO : Retrieve a product by ID without removing it.
        ...
        try:
            ...
            print(f"A product with the Product ID : {product_id} was not found")


    def sell(self, product_id: int, quantity: int) -> None:
        """TODO :Sell units of a product, reducing its stock.
        ...
        """ 
        try:
            product = Inventory.get_product(self, product_id)
        except KeyError:
            raise ProductNotFoundError(product_id)
    ...
    def restock(self, product_id: int, quantity: int) -> None:
        """TODO :Add stock to an existing product.
    ...
    def search(self, keyword: str) -> list[Product]:
        ...
        """
        return [product for product in Inventory.products.values() if (keyword in product)]
    ...
    def by_category(self, category: str) -> list[Product]:
        ...
        """
        return [product for product in Inventory.products.values() if product.category.lower()==category.lower()]
    ...
    def in_stock(self) -> list[Product]:
        ...
        """
        return [product for product in Inventory.products.values() if bool(product)]
    ...
    def price_range(self, min_price: float, max_price: float) -> list[Product]:
        ...
        """
        return [product for product in Inventory.products.values() if product >= min_price and product <= max_price]
    ...
    def summary(self) -> dict:
        ...
        total_value = sum(product.price * product.stock for product in Inventory.products.values())
        categories = sorted(set(Inventory.products))
        out_of_stock_count = [product for product in self.product.values() if not bool(product)]
